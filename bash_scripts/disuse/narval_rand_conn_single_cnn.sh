...
# NEGATIVE TRANSFER STUDY - CNN BASELINE - RAND SPLIT SENSITIVITY ANALYSIS
...
# OUT PATH
p_out_parent='/home/harveyaa/projects/def-pbellec/harveyaa/neuropsych_mtl/MTL/results/neg_transfer_rand/cnn/single/'
...
echo 'Single task CNN on conn w/ random test sets on '$task
...
do
    ...
    python $hps_balanced --tasks $task --type 'conn' --num_epochs 100 --batch_size 8 --encoder 0 --head 0 --data_format 1 --log_dir $p_out --id_dir $id_dir --data_dir $data_dir --fold $fold --rand_test
done