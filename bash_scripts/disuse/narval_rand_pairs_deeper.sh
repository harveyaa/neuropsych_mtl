#!/bin/bash
#SBATCH --job-name=pairs_conn_deeper_rand
...
# NEGATIVE TRANSFER STUDY - MLPconn_deeper PAIRWISE - RAND SPLIT SENSITIVITY ANALYSIS
...
# OUT PATH
p_out_parent='/home/harveyaa/projects/def-pbellec/harveyaa/neuropsych_mtl/MTL/results/neg_transfer_rand/deeper/'
...
echo 'paired task MLPconn_deeper on conn w/ random test sets on '$task
...
do
    ...
    python $hps_balanced --tasks $tasks --type 'conn' --num_epochs 100 --batch_size 8 --encoder 8 --head 8 --data_format 0 --log_dir $p_out --id_dir $id_dir --data_dir $data_dir --fold $fold --rand_test
done