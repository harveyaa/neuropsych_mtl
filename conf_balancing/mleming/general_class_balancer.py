...
def class_balance(classes,confounds,plim = 0.05,recurse=True,exclude_none=True,unique_classes = None,min_members=5):
	...
	while min(p_vals) < plim and np.sum(selection) > 0:
		...
		for i in rr:
			...
			if not isinstance(confounds[i,0],str):
				...
				if np.any(list(map(lambda k: len(k) < min_members, ts))):# or len(ts) <= 1:
					selection = np.zeros(classes.shape,dtype=bool)
					break
				#print("HERE")
